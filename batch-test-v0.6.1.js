(() => {
  ...
  const state = { files: [], results: [], running: false, stopRequested: false, appReady: false, wakeLock: null };
  ...
  function setProgress(done, total) { els.progress.style.width = `${total ? Math.round(done / total * 100) : 0}%`; }
  ...
  async function ensureAppReady(forceReload = false) {
    if (forceReload) {
      ...
      els.frame.src = `./index.html?batch=061&reload=${Date.now()}`;
    }
    const ready = await waitFor(() => {
      const { win, doc } = frameContext();
      const fileInput = doc.querySelector('#pdfFile');
      const marker = win.__MCCD_BATCH_READY__;
      return fileInput && marker?.version === EXPECTED_VERSION ? { win, doc } : null;
  ...
  async function processOne(file, index, total) {
    ...
    const { win, doc } = await ensureAppReady(false);
    win.__MCCD_BATCH_REVIEW__ = null;

    const clear = doc.querySelector('#clearDocument');
    if (clear && !clear.disabled) {
      clear.click();
      await waitFor(() => doc.querySelector('#placementCard')?.classList.contains('hidden'), 10000, 'document clear');
    }

    const input = doc.querySelector('#pdfFile');
    ...
    try {
      ...
      for (let i = 0; i < state.files.length; i += 1) {
        ...
        try {
          ...
          console.error(error);
          const result = { index: i + 1, sourceName: state.files[i].name, outputName: '', tone: 'error', badge: 'ERROR', detectorTitle: '', detectorStatus: '', placement: '', plan: '', seconds: 0, pdfBytes: null, thumbnails: [], error: error.message || String(error) };
          state.results.push(result); addResultCard(result);
  ...
  async function downloadResults() {
    ...
    try {
      ...
      a.href = url; a.download = `MCCDSigner-v0.6.1-batch-results-${new Date().toISOString().slice(0,10)}.zip`; document.body.append(a); a.click(); a.remove();
