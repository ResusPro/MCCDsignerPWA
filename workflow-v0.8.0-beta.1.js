...
async function importSignature(file){
  if(!file)return;
  if(file.type!=="image/png"&&file.type!=="image/jpeg")throw new Error("Use a PNG or JPEG signature image.");
  var p=window.__MCCD_APP_API__.getProfile();
...
async function forgetProfile(){
  ...
  setStatus("wfProfileStatus","Saved local profile removed. Dummy signature restored.","warning");
}
...
async function approveWorkflow(){
  ...
  var signedName=await availableName(signed,doc.reviewFileName||state.source.name.replace(/\.pdf$/i,"")+"-PWA-TEST.pdf");
...
function installUi(){
  ...
  privacy.insertAdjacentHTML("afterend",
    ...
    '<label class="file-button secondary">Choose signature image<input id="wfSignatureFile" type="file" accept="image/png,image/jpeg"></label>'+
    ...
    '<div class="wf-panel"><h3>OneDrive-synced folders</h3>'+
    ...
    '<div class="wf-incoming"><h3>Incoming MCCD</h3><div class="wf-open-row"><select id="wfPdfSelect"><option value="">Choose Incoming folder first</option></select>'+
...
window.__MCCD_WORKFLOW_INIT__=async function(){
  installUi();
  await Promise.allSettled([restoreProfile(),restoreFolders()]);
  if(!window.showDirectoryPicker)setStatus("wfFolderStatus","Remembered folder workflow is unavailable in this browser. The standard Choose PDF / share-download workflow still works.","warning");
};
})();