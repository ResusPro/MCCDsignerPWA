(() => {
  ...
  async function ensureAppReady(forceReload = false) {
    if (forceReload) {
      els.frame.src = `./processor-v0.7.0.html?batch=070&reload=${Date.now()}`;
    }
    return waitFor(() => {
  ...
  async function processOne(file, index, total) {
    ...
    try {
      ...
      setStatus(`Test ${index + 1}/${total}: ${file.name} — waiting for v0.7.0 automatic detection…`);
  ...
  function reportHtml() {
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><title>MCCDSigner v0.7.0 regression report</title>
    ...
    </head><body><header><h1>MCCDSigner v0.7.0 baseline regression report</h1>
    <p>Generated ${escapeHtml(generated)}. The production v0.7.0 detector was not modified for this run.</p>
  ...
  function downloadReport() {
    ...
    a.href = url;
    a.download = `MCCDSigner-v0.7.0-baseline-regression-${new Date().toISOString().slice(0,10)}.html`;
