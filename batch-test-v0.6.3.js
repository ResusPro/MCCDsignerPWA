(() => {
  ...
  async function ensureAppReady(forceReload = false) {
    if (forceReload) {
      ...
      els.frame.src = `./processor-v0.6.3.html?batch=063&reload=${Date.now()}`;
    }
    const ready = await waitFor(() => {
      const { win, doc } = frameContext();
      const marker = win.__MCCD_BATCH_READY__;
      const api = win.__MCCD_BATCH_API__;
      return marker?.version === EXPECTED_VERSION && api?.version === EXPECTED_VERSION && typeof api.open === 'function' && typeof api.detect === 'function' && typeof api.review === 'function' ? { win, doc, api } : null;
  ...
  async function processOne(file, index, total) {
    ...
    try {
      ...
      setStatus(`Test ${index + 1}/${total}: ${file.name} - opening PDF…`);
      phaseRef.value = 'opening';
      await api.clear();
      const bytes = await file.arrayBuffer();
      await Promise.race([
        api.open(bytes, file.name),
        new Promise((_, reject) => setTimeout(() => reject(new Error(`${file.name} PDF open timed out`)), 120000))
      ]);

      phaseRef.value = 'detection';
      setStatus(`Test ${index + 1}/${total}: ${file.name} - running automatic detection…`);
      const detected = await Promise.race([
        api.detect(),
        new Promise((_, reject) => setTimeout(() => reject(new Error(`${file.name} automatic detection timed out`)), 240000))
      ]);

      const plan = (detected.plan || 'No document plan returned.').trim();
      const detectorTitle = (detected.detectorTitle || '').trim();
      const detectorStatus = (detected.detectorStatus || '').trim();
      const badge = (detected.detectorBadge || '').trim();
      const placement = (detected.placement || '').trim();
      ...
      const review = await Promise.race([
        api.review(),
        new Promise((_, reject) => setTimeout(() => reject(new Error(`${file.name} review PDF generation timed out`)), 120000))
      ]);
      ...
      const pdfBytes = Uint8Array.from(review.bytes);
      const thumbs = [];
      for (const canvas of canvases) thumbs.push(await canvasToJpeg(canvas));

      const result = {
        index: index + 1, sourceName: file.name, outputName: review.fileName,
  ...
  async function downloadResults() {
    ...
    try {
      ...
      a.href = url; a.download = `MCCDSigner-v0.6.3-batch-results-${new Date().toISOString().slice(0,10)}.zip`; document.body.append(a); a.click(); a.remove();
